...
public class DevicesFragment extends ListFragment {
    ...
    private void startScan() {
        ...
        if (Build.VERSION.SDK_INT >= 31) {
            ...
            if (getActivity().checkSelfPermission("android.permission.BLUETOOTH_SCAN") != PackageManager.PERMISSION_GRANTED) {
                scanState = ScanState.NONE;
                AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
                builder.setTitle("BLUETOOTH_SCAN permission requested");
                builder.setMessage("BLUETOOTH_SCAN");
                builder.setPositiveButton(android.R.string.ok,
                        (dialog, which) -> requestPermissions(new String[]{"android.permission.BLUETOOTH_SCAN"}, 0));
                builder.show();
                return;

            }
            if (getActivity().checkSelfPermission("android.permission.BLUETOOTH_CONNECT") != PackageManager.PERMISSION_GRANTED) {
                scanState = ScanState.NONE;
                AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
                builder.setTitle("BLUETOOTH_CONNECT permission requested");
                builder.setMessage("BLUETOOTH_CONNECT");
                builder.setPositiveButton(android.R.string.ok,
                        (dialog, which) -> requestPermissions(new String[]{"android.permission.BLUETOOTH_CONNECT"}, 0));
                builder.show();
                return;
            }
    ...
    public void onListItemClick(@NonNull ListView l, @NonNull View v, int position, long id) {
      ...
      /*  BluetoothDevice device = listItems.get(position-1);
        ...
        // test.startService(device.getAddress());
